...
class CloudProviderTag(object):
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `CloudProviderTag`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(CloudProviderTag, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, CloudProviderTag):
            return False
