...
class CertificateUse(object):
    ...
    def __init__(
        ...
        Keyword args:
            ...
            group (FixedReferenceWithType): A reference to a certificate group that is being used, if any, where this certificate is a member of the certificate-group. This field is `null` if the referenced use object is not using a group, but is rather using this certificate directly. 
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `CertificateUse`".format(key))
