...
class CloudCapacityStep(object):
    """
    Attributes:
    ...
    def __init__(
        self,
        supported_capacity=None,  # type: int
    ):
        """
        Keyword args:
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `CloudCapacityStep`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(CloudCapacityStep, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, CloudCapacityStep):
            return False
