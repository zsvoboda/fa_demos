...
class ResourceSpaceGetResponse(object):
    ...
    def __init__(
        ...
        Keyword args:
            ...
            items (list[ResourceSpace]): Returns a list of all items after filtering. The values are displayed for each name where meaningful. If `total_only=true`, the `items` list will be empty. 
            total (list[ResourceSpace]): The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful. 
        """
