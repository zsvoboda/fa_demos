...
class CloudConfigModelPatch(object):
    """
    Attributes:
    ...
    def __init__(
        self,
        add_override_checks=None,  # type: List[str]
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `CloudConfigModelPatch`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(CloudConfigModelPatch, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, CloudConfigModelPatch):
            return False
