...
class SMISApi(object):
    ...
    def api23_smi_s_get_with_http_info(
        ...
        if 'limit' in params and params['limit'] < 0:
            raise ValueError("Invalid value for parameter `limit` when calling `api23_smi_s_get`, must be a value greater than or equal to `0`")
    ...
    def api23_smi_s_patch_with_http_info(
        self,
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.api23_smi_s_patch_with_http_info(smi_s, async_req=True)
        >>> result = thread.get()
        ...
        if smi_s is None:
            raise TypeError("Missing the required parameter `smi_s` when calling `api23_smi_s_patch`")
