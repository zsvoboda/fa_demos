...
class VolumePostBase(object):
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `VolumePostBase`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(VolumePostBase, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, VolumePostBase):
            return False
