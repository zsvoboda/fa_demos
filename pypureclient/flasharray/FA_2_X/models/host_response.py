...
class HostResponse(object):
    ...
    def __init__(
        self,
        items=None,  # type: List[models.Host]
    ):
        """
        Keyword args:
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `HostResponse`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(HostResponse, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, HostResponse):
            return False
