...
class ResourcePerformanceNoIdByArray(object):
    """
    Attributes:
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `ResourcePerformanceNoIdByArray`".format(key))
        ...
        if key == "service_usec_per_read_op_cache_reduction" and value is not None:
            if value > 1:
                raise ValueError("Invalid value for `service_usec_per_read_op_cache_reduction`, value must be less than or equal to `1`")
            if value < 0:
                raise ValueError("Invalid value for `service_usec_per_read_op_cache_reduction`, must be a value greater than or equal to `0`")
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(ResourcePerformanceNoIdByArray, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ResourcePerformanceNoIdByArray):
            return False
