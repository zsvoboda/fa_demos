...
class DirectoryGroup(object):
    ...
    def __init__(
        ...
        Keyword args:
            ...
            group (UserGroup): A group which has content in the managed directory.
            limited_by (UserGroupQuotaLimitedBy): The user-group-quota or quota policy that is limiting usage for this group in this managed directory. This policy defines the total amount of space allowed for this group or this managed directory. The returned value contains two parts&#58; the name of the policy and the managed directory to which the policy is attached. 
            path (str): Absolute path of the managed directory in the file system. 
            space (UserGroupSpace): The logical space owned by the group.
        """
        if id is not None:
            self.id = id
        if name is not None:
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `DirectoryGroup`".format(key))
