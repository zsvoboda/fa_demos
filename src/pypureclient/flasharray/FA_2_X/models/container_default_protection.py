...
class ContainerDefaultProtection(object):
    """
    Attributes:
    ...
    def __init__(
        ...
        Keyword args:
            ...
            default_protections (list[DefaultProtectionReference]): The default protection that is applied to newly created volumes or managed directories. Pgroups and policies in the list will protect volumes and managed directories, respectively. Volumes and managed directories can opt out of the default protection at creation. The pod's `default_protections` defaults to the array's `default_protections` at pod creation. To clear the list of default protections, set to an empty list `[]`. 
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `ContainerDefaultProtection`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(ContainerDefaultProtection, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ContainerDefaultProtection):
            return False
