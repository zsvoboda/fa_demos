...
class PolicyLockable(object):
    ...
    swagger_types = {
        ...
        'policy_type': 'str',
        'time_remaining': 'int',
        'eradication_config': 'PolicyLockableEradicationConfig',
        'retention_lock': 'str'
    }
    ...
    def __init__(
        ...
        eradication_config=None,  # type: models.PolicyLockableEradicationConfig
        ...
        Keyword args:
            ...
            eradication_config (PolicyLockableEradicationConfig)
            retention_lock (str): The valid values are `ratcheted` and `unlocked`. The default value for a newly created policy is `unlocked`. Set `retention_lock` to `ratcheted` to enable SafeMode restrictions on the policy. Contact Pure Technical Services to change `retention_lock` to `unlocked`. 
        """
        if id is not None:
            self.id = id
        if name is not None:
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `PolicyLockable`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(PolicyLockable, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, PolicyLockable):
            return False
