...
class SnmpV2c(object):
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `SnmpV2c`".format(key))
        if key == "community" and value is not None:
            if len(value) > 32:
                raise ValueError("Invalid value for `community`, length must be less than or equal to `32`")
