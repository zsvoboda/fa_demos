...
class DirectoryServicePatch(object):
    """
    Attributes:
    ...
    def __init__(
        ...
        """
        Keyword args:
            name (str): The new name for the resource. The name is expected to be fully qualified, meaning if the object is contained in some context, the corresponding name would provide complete information about the containment hierarchy. For example, `name01,realm01::name01,realm01::pod01::name01`. 
            ...
            management (DirectoryServiceManagement)
            sources (list[Reference]): List of network interfaces used for egress. For directory services at the array level (not in any Realm), this list can be empty. If the list is empty, the system will look use VIFs with the \"ds\" service (or if none, the \"management\" service) for egress. 
        """
        if name is not None:
            self.name = name
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `DirectoryServicePatch`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(DirectoryServicePatch, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, DirectoryServicePatch):
            return False
