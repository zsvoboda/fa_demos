...
class ActiveDirectoryPost(object):
    ...
    def __init__(
        ...
        Keyword args:
            ...
            service_principal_names (list[str]): A list of service principal names to register for the machine account, which can be used for the creation of keys for Kerberos authentication. If not specified, the default `service_principal_names` are constructed using the `computer_name` and `domain` fields. 
