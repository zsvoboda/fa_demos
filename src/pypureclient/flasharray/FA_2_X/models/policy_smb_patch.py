...
class PolicySmbPatch(object):
    ...
    def __init__(
        ...
        Keyword args:
            ...
            continuous_availability_enabled (bool): If set to `true`, enables continuous availability on the policy. When continuous availability is enabled on a policy, file shares are accessible during otherwise disruptive scenarios such as temporary network outages, controller upgrades or failovers. 
        """
        if name is not None:
            self.name = name
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `PolicySmbPatch`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(PolicySmbPatch, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, PolicySmbPatch):
            return False
