...
class ResourcePodSpaceGetResponse(object):
    """
    Attributes:
    ...
    swagger_types = {
        ...
        'items': 'list[ResourcePodSpace]',
        'total': 'list[ResourcePodSpace]'
    }
    ...
    def __init__(
        ...
        items=None,  # type: List[models.ResourcePodSpace]
        total=None,  # type: List[models.ResourcePodSpace]
    ):
        """
        Keyword args:
            ...
            items (list[ResourcePodSpace]): Displays a list of all items after filtering. The values are displayed for each name where meaningful. If `total_only=true`, the `items` list will be empty. 
            total (list[ResourcePodSpace]): The aggregate value of all items after filtering. When applicable, the average value is displayed instead. The values are displayed for each field if meaningful. 
        """
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `ResourcePodSpaceGetResponse`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(ResourcePodSpaceGetResponse, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ResourcePodSpaceGetResponse):
            return False
