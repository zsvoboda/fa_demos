...
class SupportRemoteAssistSessionsPatch(object):
    """
    Attributes:
    ...
    def __init__(
        ...
        """
        Keyword args:
            access_level_override (str): Overrides the default access level for this RA session only. Values include `restricted` and `elevated`. If not provided, the remoteassist session will use the default_access_level setting. 
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `SupportRemoteAssistSessionsPatch`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(SupportRemoteAssistSessionsPatch, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, SupportRemoteAssistSessionsPatch):
            return False
