...
class HardwareApi(object):
    ...
    def api220_hardware_get_with_http_info(
        self,
        ...
        >>> thread = api.api220_hardware_get_with_http_info(async_req=True)
        >>> result = thread.get()
        ...
        if 'limit' in params and params['limit'] < 0:
            raise ValueError("Invalid value for parameter `limit` when calling `api220_hardware_get`, must be a value greater than or equal to `0`")
        if 'offset' in params and params['offset'] < 0:
            raise ValueError("Invalid value for parameter `offset` when calling `api220_hardware_get`, must be a value greater than or equal to `0`")
        collection_formats = {}
        path_params = {}
        ...
        return self.api_client.call_api(
            ...
            response_type='HardwareGetResponse',
    ...
    def api220_hardware_patch_with_http_info(
        self,
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.api220_hardware_patch_with_http_info(hardware, async_req=True)
        >>> result = thread.get()

        :param HardwarePatch hardware: (required)
        :param str authorization: Access token (in JWT format) required to use any API endpoint (except `/oauth2`, `/login`, and `/logout`) 
        :param str x_request_id: Supplied by client during request or generated by server. 
        :param list[str] names: Performs the operation on the unique name specified. Enter multiple names in comma-separated format. For example, `name01,name02`. 
        :param bool async_req: Request runs in separate thread and method returns multiprocessing.pool.ApplyResult.
        ...
        if hardware is None:
            raise TypeError("Missing the required parameter `hardware` when calling `api220_hardware_patch`")
