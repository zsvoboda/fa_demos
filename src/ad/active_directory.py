...
class ActiveDirectory:
    ...
    def ensure_ou_exists(self, ou_name):
        ...
        if self.conn.entries:
            _logger.warning(f"'{ou_name}' already exists in Active Directory. Skipping creation.")
            return

        _logger.info(f"Creating Organizational Unit: {ou_name}")
        self.conn.add(ou_dn, 'organizationalUnit')

        if self.conn.result['description'] == 'success':
            _logger.info(f"Organizational Unit '{ou_name}' created successfully.")
        else:
            _logger.error(f"Failed to create Organizational Unit '{ou_name}':", self.conn.result)
            raise RuntimeError(f"Failed to create Organizational Unit '{ou_name}'.")
    ...
    def create_group(self, name, gid, description=''):
        ...
        if self.conn.entries:
            _logger.warning(f"Group '{name}' already exists. Skipping creation.")
            return
        ...
        if self.conn.result['description'] == 'success':
            _logger.info(f"Group '{name}' created successfully.")
        else:
            _logger.error(f"Failed to create group '{name}':", self.conn.result)
    ...
    def create_user(self, name, uid, description=''):
        ...
        if self.conn.entries:
            _logger.warning(f"User '{name}' already exists. Skipping creation.")
            return
        ...
        if self.conn.result['description'] != 'success':
            _logger.error(f"Failed to create user '{name}':", self.conn.result)
            raise RuntimeError(f"Failed to create user '{name}'.")

        _logger.info(f"User '{name}' created successfully.")
        ...
        if self.conn.result['description'] != 'success':
            _logger.error(f"Failed to set password for '{name}':", self.conn.result)
            raise RuntimeError(f"Failed to set password for '{name}'.")

        _logger.info(f"Password set successfully for user '{name}'.")

        # Enable account
        self.conn.modify(user_dn, {'userAccountControl': [(MODIFY_REPLACE, [512])]})
        _logger.info(f"User '{name}' enabled successfully.")
    ...
    def add_user_to_group(self, group_name, user_name):
        ...
        if not self.conn.entries:
            _logger.warning(f"Group '{group_name}' not found. Skipping.")
            return

        self.conn.search(self.base_dn, f'(distinguishedName={user_dn})', attributes=['distinguishedName'])
        if not self.conn.entries:
            _logger.warning(f"User '{user_name}' not found. Skipping.")
            return

        self.conn.modify(group_dn, {'member': [(MODIFY_REPLACE, [user_dn])]})
        _logger.info(f"User '{user_name}' added to group '{group_name}'.")
    ...
    def delete_object(self, dn, object_type):
        ...
        if not self.conn.entries:
            _logger.warning(f"{object_type} '{dn}' not found. Skipping deletion.")
            return

        self.conn.delete(dn)
        _logger.info(f"{object_type} '{dn}' deleted successfully.")
    ...
    def close(self):
        ...
        self.conn.unbind()
        _logger.info("AD connection closed.")